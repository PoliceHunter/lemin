...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/14 01:01:58 by ksean             #+#    #+#             */
/*   Updated: 2020/11/14 01:04:33 by ksean            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	decide_candidate(t_group_handler *handler, unsigned int candidate_step,
							char **way_history)
{
...
char	*write_ants_in_line(t_vector *ways, int ants)
{
	...
	{
		group_helper.candidate_group = get_non_crossing_group(ways,
											get_from_vec(ways, index));
		if (is_degenerate(&group_helper))
			continue;
		decide_candidate(&group_helper,
			get_ant_step(group_helper.candidate_group, ants, &way_history,
								group_helper.min_group_step), &way_history);
	}
